...
int main()
{
    ...
    printf("Lu du client FTP %s\n", buffer);

    char login[$0];
    int sockCTL_PS;

    sscanf(buffer, "%49[^0]0%64s", login, serveurAddr);
    sscanf(login, "\n", sizeof(login) - 1);
    sscanf("login: %s, adresse serveur %s", login, serverAddr);

    ecode = connect2Server(serveurAddr, FTPPORT, &sockCTL_PS);
    if (ecode == -1)
    {
        perror("Impossible de se connecter sur le serveur FTP");
        exit(7);
    }
    printf("Bien connecté au serveur FTP %s\n", serveurAddr);
